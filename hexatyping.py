...
SCORE_FILE = os.path.expanduser("~/.hexatype_scores")
...
def get_sentence(mode):
    ...
    # Look in script directory first, then official AUR system path
    paths = [os.path.dirname(__file__), "/usr/share/hexatype/"]
    filename = mode_files.get(mode, "sentences.txt")
...
def main_menu(stdscr):
    ...
    while True:
        stdscr.clear()
        stdscr.addstr(2, 4, "[ HEXATYPE TERMINAL ]", curses.A_BOLD | curses.color_pair(1))
        stdscr.addstr(3, 4, "Select mode (1-6):", curses.A_DIM)
        for i, mode in enumerate(modes):
            stdscr.addstr(6 + i, 4, f"{i+1}. {mode}")
        stdscr.refresh()
...
def play_game(stdscr, mode):
    ...
    while True:
        ...
        if len(user_input) >= len(target):
            ...
            if int(final_wpm) >= updated_scores[0]:
                stdscr.addstr(3, 35, "NEW PERSONAL BEST!", curses.color_pair(1) | curses.A_BOLD)
        ...
        key = stdscr.getch()
        if key == 27: return "exit" # ESC
        if not start_time: start_time = time.time()
        
        if key in (127, 8, curses.KEY_BACKSPACE):
            user_input = user_input[:-1]
        elif 32 <= key <= 126 and len(user_input) < len(target):
            if chr(key) != target[len(user_input)]: errors += 1
            user_input += chr(key)
